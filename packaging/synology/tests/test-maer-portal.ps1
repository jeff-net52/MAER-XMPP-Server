...
Require ($portal.Contains('ejabberd_hooks:add(set_password, Host, ?MODULE, password_changed, 45)')) 'Portal sessions are not connected to the ejabberd password-change lifecycle.'
Require ($portal.Contains('ejabberd_hooks:add(remove_user, Host, ?MODULE, account_removed, 45)')) 'Portal metadata is not connected to the ejabberd account-removal lifecycle.'
Require ($portal.Contains('ejabberd_hooks:delete(set_password, Host, ?MODULE, password_changed, 45)')) 'Portal password-change hook is not removed during shutdown.'
Require ($portal.Contains('ejabberd_hooks:delete(remove_user, Host, ?MODULE, account_removed, 45)')) 'Portal account-removal hook is not removed during shutdown.'
Require ($portal.Contains('Pid when Pid =:= self() -> ok')) 'Portal hook delivery does not prevent a recursive gen_server call during self-service password changes.'
Require ($portal.Contains('DELETE FROM portal_tokens WHERE username = ?')) 'Password changes and account removals do not revoke all persistent portal tokens.'
Require ($portal.Contains('DELETE FROM portal_profiles WHERE username = ?')) 'Deleted accounts can retain portal metadata for a reused localpart.'
Require ($portal.Contains('password_change_revokes_sessions_and_tokens_test')) 'Password-change lifecycle regression coverage is missing.'
Require ($portal.Contains('account_deletion_then_recreation_is_clean_test')) 'Delete-and-recreate lifecycle regression coverage is missing.'
...
Require ($portal.Contains('{preauth_tokens_per_ip, 16}')) 'Per-IP pre-authentication token capacity is not bounded.'
Require ($portal.Contains('{preauth_tokens_global, 2048}')) 'Global pre-authentication token capacity is not bounded.'
Require ($portal.Contains('cleanup_expired_preauth(State#state.preauth, Now)')) 'Expired pre-authentication tokens are not purged before capacity checks.'
Require ($portal.Contains('GlobalCount < GlobalLimit andalso PerIPCount < PerIPLimit')) 'Pre-authentication allocation does not enforce both capacity limits atomically.'
Require ($portal.Contains('{error, rate_limited} -> preauth_rate_limited()')) 'Exhausted pre-authentication capacity is not exposed as HTTP 429.'
Require ($portal.Contains('preauth_quota_and_expiry_test')) 'Pre-authentication quota/expiry regression coverage is missing.'
